...
public class MainActivity extends Activity implements AudioListener.AudioListenerCallback {
    ...
    public void clickedSwitchCamera(View view) {
		...
		final SensorEventListener mSensorListener = new SensorEventListener() {
			...
			{
				return ""+pressure_value+" BarA: "+height;
			}
		};
		...
		new Thread() {
			public void run() {
				int delay=0;		//10 sec *delay
				final int maxBatteryHeat=60;
				final String phone = "0505770971";
				while (true) {
					try {
						runOnUiThread(new Runnable() {
							...
							@Override
							public void run() {
								if(batteryTemperature()<maxBatteryHeat) {
									View takePhotoButton = findViewById(net.sourceforge.opencamera.R.id.take_photo);
						...
						{
							try {
								double latitude = MainActivity.this.getLocationSupplier().getLocation().getLatitude();
								double longitude = MainActivity.this.getLocationSupplier().getLocation().getLongitude();
								double altitude = 0;
								if(MainActivity.this.getLocationSupplier().getLocation().hasAltitude())
									altitude = MainActivity.this.getLocationSupplier().getLocation().getAltitude();
								DecimalFormat numberFormat = new DecimalFormat("#.000000");
								String msg = "Lat: " + numberFormat.format(latitude) + " Long: " + numberFormat.format(longitude) + " GPSA: " + numberFormat.format(altitude) + " Pressure: " + mSensorListener.toString() + " Battery: " + batteryLevel() + "% Battarey Temp: " + batteryTemperature();
